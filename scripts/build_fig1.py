...
def build(plt, mpl, np, ctx):
    ...
    # wspace is the only thing keeping C's long row labels off panel B/E; 0.24
    # leaves a measured ~0.01 figure-width gutter, verified in align_bottoms().
    outer = fig.add_gridspec(1, 2, width_ratios=[2.45, 1.0], wspace=0.30)
    # top row: two UMAPs of the same embedding, kept equal in size so they stay
    # visually comparable; bottom row: D narrower, E wider so its ten metric
    # labels do not collide.
    left = outer[0].subgridspec(2, 1, hspace=0.42)
    top = left[0].subgridspec(1, 2, wspace=0.24)
    # E carries ten rotated metric labels and needs more width than D; the row is
    # still flush left and right and the two frames are levelled after the draw
    bot_gs = left[1].subgridspec(1, 2, width_ratios=[0.78, 1.22], wspace=0.42)
    ...
    for l, (dx, dy) in g["OFF"].items():
        x0, y0 = g["cent"][l]
        axA.annotate(f"{g['LINLAB'][l]} ({counts[l]:,})", xy=(x0, y0),
    ...
    for ax, ttl in [(axA, "Eight lineages resolved across 344,510 cells"),
                    (axB, "Cohorts intermix after integration (quantified in E)")]:
    ...
    axC.set_yticks(ypos)
    axC.set_yticklabels([l.replace(" — ", "\n") for l in labels], fontsize=TIER[2])
    axC.invert_yaxis(); axC.set_xlim(0, 100); axC.set_ylim(len(rows_t) + 2.6, -0.8)
    axC.set_xlabel("Cells (%)")
    axC.set_title("Composition by site and disease status", loc="center", pad=10)
    axC.text(1.005, -0.048, "cells · samples", transform=axC.transAxes, fontsize=TIER[2],
             color="0.35", ha="left")
    lgC = axC.legend(frameon=False, fontsize=TIER[1], ncol=2, loc="upper center",
                     bbox_to_anchor=(0.46, -0.075), handlelength=.9, columnspacing=.9, handletextpad=.4)
    ...
    axD.set_title("Cohort contributions (cells · samples; 63 in total)", loc="center", pad=14)
    ...
    axE.bar(x + w / 2, sb.loc["Harmony", cols].values, w, color="#1F6FB2", label="Harmony (batch = sample)")
    ...
    axE.set_title("Batch mixing improves without loss of lineage structure", loc="center", pad=14)
    ...
    axE.text(len(bio) + 1.5, 1.008, "batch correction", ha="center", fontsize=TIER[2],
             color=MG, transform=axE.get_xaxis_transform())
    lgE = axE.legend(frameon=False, loc="upper center", bbox_to_anchor=(0.5, -0.30), ncol=2,
