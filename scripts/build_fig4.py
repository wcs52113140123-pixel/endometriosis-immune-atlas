...
def _circle_plot(ax, net, focus, plt, np, top_edges=60):
    ...
    for c in cts:
        ...
        ax.text(x * 1.13, y * (1.22 if near_v else 1.13), SHORT.get(c, c), fontsize=TIER[1],
...
def _chord(ax, ser, focus, plt, np):
    ...
    for c in send:
        ...
        ax.text(1.13 * np.cos(np.deg2rad(mid)),
                (1.26 if near_v else 1.13) * np.sin(np.deg2rad(mid)),
...
def build(plt, np, pd, ctx):
    ...
    fig = plt.figure(figsize=(11.8, 13.2))
    ...
    gs = fig.add_gridspec(3, 2, hspace=0.52, wspace=0.30, width_ratios=[1.0, 1.0],
                          height_ratios=[1.30, 1.0, 1.0],
                          left=0.075, right=0.975, top=0.945, bottom=0.045)
    ...
    axC.set_xlim(-.7, len(senders) - .3); axC.set_ylim(-.9, len(pairs) + 1.5)
    ...
    axC.legend(frameon=False, fontsize=TIER[2], loc="upper left", bbox_to_anchor=(-0.005, 1.03),
               ncol=2, handletextpad=.4, columnspacing=0.8,
               title="−log$_{10}$ specificity rank", title_fontsize=TIER[2])
    # colour bar inside the reserved top band: below the panel it read as a
    # detached strip at the bottom of the whole figure
    cbC = axC.inset_axes([0.63, 0.955, 0.33, 0.028])
    cb = fig.colorbar(scC, cax=cbC, orientation="horizontal")
    cb.set_label("expression mean", fontsize=TIER[2], labelpad=2)
    ...
    axD.text(0.99, 0.03, "number at bar end = interactions", transform=axD.transAxes,
             ha="right", fontsize=TIER[2], color=MG)
    ...
    lim = float(max(role.outgoing.max(), role.incoming.max())) * 1.12
    axE.plot([0, lim], [0, lim], ls="--", lw=.7, color="0.72", zorder=1)
    # labels pushed away from the identity line, with a white pad: at a fixed
    # offset they sat on top of their own points and on each other
    annsE = []
    for c in role.index:
        x, y = role.loc[c, "outgoing"], role.loc[c, "incoming"]
        # a point near the left edge always gets its label on the right, or the
        # up-left placement runs into the y axis tick labels
        up = (y >= x) and x > 0.12 * float(role.outgoing.max())
        dx, dy = (-6, 7) if up else (7, -7)
        annsE.append(axE.annotate(SHORT.get(c, c), (x, y), xytext=(dx, dy), textcoords="offset points",
                     fontsize=TIER[2], ha="right" if up else "left",
                     va="bottom" if up else "top",
                     color="#C0392B" if c == "Treg" else "0.3",
                     fontweight="bold" if c == "Treg" else "normal",
                     bbox=dict(fc="white", ec="none", alpha=.72, pad=0.6)))
    ...
    axE.text(0.03, 0.95, "point size = number of interactions", transform=axE.transAxes,
             fontsize=TIER[2], color=MG, va="top")
    ...
    for i, gn in enumerate(genes):
        for j, st in enumerate(states):
            ...
            ss.append(float(fe.loc[st, gn]) * 55); cs.append(float(me.loc[st, gn]))
    scF = axF.scatter(xs, ys, s=ss, c=cs, cmap="Reds", ec="0.45", lw=.25)
    axF.set_xticks(range(len(states)))
    axF.set_xticklabels([SHORT.get(c, c) for c in states], rotation=42, ha="right")
    axF.set_yticks(range(len(genes))); axF.set_yticklabels(genes)
    axF.set_xlim(-.7, len(states) - .3); axF.set_ylim(-.9, len(genes) + 1.8)
    axF.set_title("Expression of the ligands and receptors involved", loc="center", pad=10)
    for s_, lab in [(0.25 * 55, "25%"), (0.75 * 55, "75%")]:
        axF.scatter([], [], s=s_, c="0.75", ec="0.45", lw=.25, label=lab)
    axF.legend(frameon=False, fontsize=TIER[2], loc="upper left", bbox_to_anchor=(-0.005, 1.03),
               ncol=2, handletextpad=.4, columnspacing=0.8, title="cells expressing",
               title_fontsize=TIER[2])
    cbF = axF.inset_axes([0.63, 0.965, 0.33, 0.022])
    cb = fig.colorbar(scF, cax=cbF, orientation="horizontal")
    cb.set_label("mean expression", fontsize=TIER[2], labelpad=2)
    cb.ax.tick_params(labelsize=TIER[2], length=2, pad=1)
    cb.ax.xaxis.set_label_position("top"); cb.ax.xaxis.set_ticks_position("bottom")
...
def fit_circles(fig, ax, keys="AB", r=1.58):
