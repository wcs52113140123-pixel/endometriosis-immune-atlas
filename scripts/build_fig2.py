...
def build(plt, np, pd, ctx):
    ...
    gs = fig.add_gridspec(2, 6, hspace=0.66, wspace=1.55)
    axA = fig.add_subplot(gs[0, 0:3]); axB = fig.add_subplot(gs[0, 3:6])
    axC = fig.add_subplot(gs[1, 0:2]); axD = fig.add_subplot(gs[1, 2:4]); axE = fig.add_subplot(gs[1, 4:6])
    ...
    axA.set_title("Per-sample Treg fractions overlap in every cohort", loc="center", pad=10)
    ...
    axB.set_title("Pooling cells inflates the difference", loc="center", pad=10)
    ...
    axC.text(0.03, 0.97, f"Spearman ρ = {rho[0]:.2f}, P = {rho[1]:.2f}", transform=axC.transAxes,
             va="top", fontsize=TIER[2], color="0.3")
    axC.set_title("Large case samples drive the pooled estimate", loc="center", pad=10)
    axC.legend(frameon=False, fontsize=TIER[2], ncol=2, loc="upper center",
               bbox_to_anchor=(0.5, -0.24), handletextpad=.3, columnspacing=1.6)
    ...
    for y, (_, r) in zip(yy, FO.iterrows()):
        ...
        axD.text(-1.42, y + 0.20, f"{r.label}  ({r.n})   P = {r.p:.2f}", fontsize=TIER[2],
                 va="bottom", ha="left", color="0.25", zorder=5,
                 bbox=dict(fc="white", ec="none", pad=1.4))
    axD.axvline(0, color=C_ENDO, lw=.9, ls="--", zorder=1)
    axD.set_yticks([])
    axD.set_xlabel("Effect on log(Treg % + 0.1),\nendometriosis − control")
    axD.set_xlim(-1.5, 1.62); axD.margins(y=0.22)
    axD.set_title("No estimate excludes zero", loc="center", pad=10)
    ...
    axE.set_title("Pooling inflates the false-positive rate", loc="center", pad=10)
