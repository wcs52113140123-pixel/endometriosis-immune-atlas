...
def place_letters(fig, ax, dy=0.030, dx=0.055):
    for a in fig.axes:
        for t in list(a.texts):
            if t.get_text() in list("ABCDEFGH") and t.get_fontweight() in ("bold", 700):
                t.remove()
    for t in list(fig.texts):
        if t.get_text() in list("ABCDEFGH") and t.get_fontweight() in ("bold", 700):
            t.remove()
    for lab in [k for k in ax if k in "ABCDEFGH"]:
...
def build(plt, np, pd, ctx):
    ...
    # PRINT-SIZE VARIANT: 180 mm journal column = 7.09 in, so 11 pt on the page
    fig = plt.figure(figsize=(7.09, 11.2))
    gs = fig.add_gridspec(4, 2, hspace=0.52, wspace=0.42,
    ...
    axG = fig.add_subplot(gs[3, 0]); axH = fig.add_subplot(gs[3, 1])
    ...
    axB.set_yticks(range(len(SIG))); axB.set_yticklabels(SIG, fontsize=TIER[2], style="italic")
    axB.set_xticks([])
    for i in range(1, 3):
    ...
    # the two strip labels are dropped: the upper strip is named by the segment
    # names written directly above it and the lower one by the tissue colour key
    # below the panel, so both labels only crowded panel A's right edge
    ...
    for k, r in lab.iterrows():
        axD.annotate(r.gene, style="italic", xy=(r.lfc, r.nlp), xytext=(2.05, ytop - k * 0.42),
    ...
    for t, s_ in zip(axE.get_xticklabels(), SEGS):
        t.set_color(SEGC[s_])
    axE.set_yticks(range(len(SIG))); axE.set_yticklabels(SIG, fontsize=TIER[2], style="italic")
    ...
    axF.margins(y=0.20)
    axF.set_title("Independent scRNA cohort (GSE247695)", loc="center", pad=16)
    # explicit in-range ticks: matplotlib keeps an off-range tick label as a live
    # Text object that never renders but does collide in the overlap audit
    _y0, _y1 = axF.get_ylim()
    axF.set_yticks([v for v in np.arange(0.1, 0.81, 0.1) if _y0 < v < _y1])
    ...
    # ---- G: sorted macrophages, endometriosis vs control eutopic endometrium ----
    # right cell type, different contrast: this compares eutopic endometrium
    # between patients and controls, not lesion against eutopic within a patient
    m130 = ctx["m130"]; t130 = ctx["t130"]
    GC = {"Control": TISC["eutopic endometrium"], "Endometriosis": TISC["endometriotic lesion"]}
    pos = 0
    xt, xl = [], []
    for srt in ["M1", "M2"]:
        for grp in ["Control", "Endometriosis"]:
            v = m130[(m130["sort"] == srt) & (m130.group == grp)].score.values
            axG.boxplot([v], positions=[pos], widths=.62, patch_artist=True, showfliers=False,
                        medianprops=dict(color="0.15", lw=1.3),
                        boxprops=dict(fc=GC[grp], ec="0.35", lw=.8, alpha=.55),
                        whiskerprops=dict(color="0.5", lw=.8), capprops=dict(color="0.5", lw=.8))
            rg = np.random.default_rng(20 + int(round(pos * 10)))
            axG.scatter(pos + rg.uniform(-.14, .14, len(v)), v, s=20, c=GC[grp], ec="w", lw=.4, zorder=3)
            xt.append(pos); xl.append(f"{srt} {grp[:4]}\n{len(v)}"); pos += 1
        pos += 0.6
    axG.set_xticks(xt); axG.set_xticklabels(xl, fontsize=TIER[2])
    axG.axhline(0, color="0.8", lw=.6, zorder=0)
    axG.set_ylabel("Immunosuppression score")
    axG.set_title("Sorted macrophages (GSE130435)", loc="center", pad=8)
    # sort written into the tick labels: above the axes the two names sat in the
    # title band
    axG.margins(y=0.22)
    axG.text(.5, .02, f"M1 P = {t130.loc[0,'p_greater']:.2f} · M2 P = {t130.loc[1,'p_greater']:.2f}",
             transform=axG.transAxes, ha="center", va="bottom", fontsize=TIER[2],
             style="italic", color="0.25")

    # ---- H: adenomyosis DSP, ectopic vs eutopic within patient ----
    # same contrast structure and platform as B-E, different disease and a
    # pan-leukocyte rather than a macrophage segment
    p303 = ctx["p303"]
    for j, (cn, col) in enumerate([("Functionalis", TISC["eutopic endometrium"]),
                                   ("Adenomyosis", TISC["endometriotic lesion"])]):
        v = p303[cn].values
        axH.boxplot([v], positions=[j], widths=.6, patch_artist=True, showfliers=False,
                    medianprops=dict(color="0.15", lw=1.3),
                    boxprops=dict(fc=col, ec="0.35", lw=.8, alpha=.55),
                    whiskerprops=dict(color="0.5", lw=.8), capprops=dict(color="0.5", lw=.8))
    for _, row in p303.iterrows():
        axH.plot([0, 1], [row["Functionalis"], row["Adenomyosis"]], color="0.62", lw=.7, zorder=2)
    for j, cn in enumerate(["Functionalis", "Adenomyosis"]):
        axH.scatter([j] * len(p303), p303[cn].values, s=20,
                    c=TISC["eutopic endometrium"] if j == 0 else TISC["endometriotic lesion"],
                    ec="w", lw=.4, zorder=3)
    axH.set_xticks([0, 1]); axH.set_xticklabels([f"Eutopic\n{len(p303)}", f"Lesion\n{len(p303)}"])
    axH.set_xlim(-.62, 1.62)
    axH.set_ylabel("Immunosuppression score")
    axH.set_title("Adenomyosis DSP, CD45 (GSE303150)", loc="center", pad=8)
    # the note lay across the paired lines. The title line is full, so instead the
    # axes gain a clear band below the data and the note sits in it, keeping the
    # same bottom-of-panel placement as F and G
    _lo, _hi = float(p303.values.min()), float(p303.values.max())
    axH.set_ylim(_lo - 0.34 * (_hi - _lo), _hi + 0.06 * (_hi - _lo))
    axH.text(.5, .02, f"{int((p303['Adenomyosis'] > p303['Functionalis']).sum())}/{len(p303)} patients up · "
             f"P = {ctx['p303_p']:.2f}", transform=axH.transAxes, ha="center", va="bottom",
             fontsize=TIER[2], style="italic", color="0.35")
    ...
    ax = dict(A=axA, B=axB, C=axC, D=axD, E=axE, F=axF, G=axG, H=axH,
              cbB=cbB, cbE=cbE, lgE=lgE)
    g["normalize"](fig)
    for a in (axB, axE):
